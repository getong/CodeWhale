...
async fn child_write_tool_fails_closed_outside_registered_scope() {
    ...
    let canonical_shell_err = registry
        .execute(
            "agent_scoped",
            "Bash",
            json!({"action": "run", "command": "touch docs/canonical-shell.txt"}),
        )
        .await
        .expect_err("canonical Bash must obey the same shared-workspace claim")
        .to_string();
    assert!(
        canonical_shell_err.contains("cannot prove a bounded file target"),
        "{canonical_shell_err}"
    );
    assert!(!tmp.path().join("docs/canonical-shell.txt").exists());
